...
- [x] `/api/transcribe` → `/api/records` が通り、保存された INDEX が返るか（実機確認済み 2026-07-20）
- [ ] アプリ再起動後もログイン状態が保持されるか（LargeSecureStore）
- [x] バックグラウンド/割り込み（電話・通知）時の録音挙動 — 実機での能動的再現が困難なためコードレビューでクローズ（2026-07-20）。`lib/useRecording.ts` の AppState ハンドラ（disarm → タイマー解除 → stop() → セッション解除 → error 表示、`armedRef`/`finishingRef` で他経路と多重実行ガード）を確認
...
- [x] 文字起こし結果が trim 後5文字未満のとき phase=error / 「短い、話せ」表示 + haptics.warning になるか（実機確認済み 2026-07-20）
- [x] `/api/transcribe` が 429 を返したとき、body の `error` フィールドに応じて login_required/daily_limit 分岐が機能するか（実機確認済み 2026-07-20）
...
- [ ] `AuthScreen` に新設した email ステップの「戻る」導線（onboarding のプレビュー画面へ復帰）でレイアウト崩れが無いか（email → select → onboarding hero の戻り遷移とレイアウトは iOS Simulator で確認済み 2026-07-20。録音プレビューが残った状態での復帰のみ未確認）
- [ ] hero 画面のマイクボタン（96px）・下部「ログイン」リンクのタップ領域とレイアウトが実機で意図通りか（レイアウト・「ログイン」タップ遷移は iOS Simulator で確認済み 2026-07-20。実機でのタップ領域の感触のみ未確認）
...
- [x] 実録音 → CARVING 突入でタイトル刻印 → ティッカー → ドット前進 → NEXT #NNN の順に演出が出るか（実機確認済み 2026-07-20）
...
- [x] リマインダーONで通知許可ダイアログが出て、許可すると指定時刻（デフォルト21:00）に毎日通知が届くか（実機確認済み 2026-07-20）
...
- [x] レポート通知ONで毎週月曜8:00・毎月1日8:00 に通知が届くか（実機確認済み 2026-07-20）
...
以下5項目は実機での再現が困難なため、コードレビューでのロジック検証をもって一旦クローズ（2026-07-20）。`App.tsx` の getSession `.catch` + `retryTick` 再実行 + 5秒タイムアウトプレースホルダ、`SettingsScreen` の signOut await + エラー表示、`largeSecureStore.test.ts` の壊れた blob → null 単体テストを確認済み。実機で問題が再現した場合に再オープンする:

- [x] 低速回線（Network Link Conditioner 等で 5 秒超の遅延を再現）でログイン済み端末を起動したとき、一瞬でも `OnboardingScreen` が表示されず、RETRY 画面 → セッション確定後に自動で `RecordScreen` へ遷移するか
- [x] RETRY 画面で "RETRY" を押すと `getSession()` が再試行され、成功すればそのまま `RecordScreen`／`OnboardingScreen` に遷移するか
- [x] 機内モード等で完全にオフラインのまま起動タイムアウトを迎えた場合、RETRY 画面から抜けられない状態が続かないか（ネットワーク復帰後に RETRY で復旧できるか）
- [x] オフライン状態で設定画面からログアウトを試みたとき、モーダルが閉じずに「ログアウトできなかった。もう一度。」が表示され、再度オンラインでログアウトを押すと正常に閉じてオンボーディングに戻るか
- [x] アプリ削除・再インストールや OS の Keychain リセット等で `expo-secure-store` のエントリが壊れた状態を作れた場合、起動がクラッシュ/無限スプラッシュにならず未ログイン扱いで先に進むか（単体テストではロジックのみ検証、実際の Keychain 破損は再現困難）
...
INSIGHT の EMOTION チャート・REPORTS カードのミニ sparkline を、グラデーション塗りから単色（`SENTIMENT_POS`/`SENTIMENT_NEG`、色値は変更なし）に変更した（`components/EmotionChart.tsx`, `components/ReportCard.tsx`）。あわせて `DESIGN.md` を yuzu-app から移植し、UI の見た目を正典と照合する `design-review` スキルを新設した。見た目のみの変更で typecheck/lint は green だが、SVG の描画結果はコードレベルでは確認できないため実機（またはExpo Go）での確認が必要:

- [ ] INSIGHT タブの EMOTION チャートで、プラス側がオレンジ・マイナス側が紺の単色塗り（グラデーションではない）で表示されるか
- [ ] REPORTS カードのミニ sparkline、レポート詳細モーダルの EMOTION チャートも同様に単色塗りになっているか
- [ ] 単色化により、グラデーションで作られていた「ゼロラインに近づくほど薄くなる」視覚効果が無くなった状態が意図通り（もし違和感があれば配色案の再検討が必要）

利用規約・プライバシーポリシーのページ公開に伴い、設定画面の LEGAL セクション（`components/SettingsScreen.tsx`、審査対策で一時削除していた）を、外部ブラウザで `https://yuzu.style/terms` / `https://yuzu.style/privacy` を開くタップ可能な行として復活させた（既存の NOTIFICATIONS/CONNECT/SUPPORT と同じ Pressable+haptics行パターンを再利用）。`Linking.openURL` の実際の遷移・タッチ領域はコードレベルでは確認できないため実機（またはExpo Go）での確認が必要:

- [ ] 設定 → LEGAL の「利用規約」をタップすると haptics が鳴り、外部ブラウザ（Safari 等）で `https://yuzu.style/terms` が開くか
- [ ] 設定 → LEGAL の「プライバシーポリシー」をタップすると同様に `https://yuzu.style/privacy` が開くか

サインイン画面（`components/AuthScreen.tsx` select ステップ）に法的同意注記「続けることで利用規約とプライバシーポリシーに同意したものとみなす」を追加し、「利用規約」「プライバシーポリシー」部分をネストした `Text` の `onPress` でインラインリンク化した（チェックボックスなし・注記のみ）。ネストTextのタップ領域はRNでは行送り・文字間隔次第で意図せず狭くなることがあるため実機（またはExpo Go）での確認が必要:

- [x] SIGN IN 画面下部の注記中「利用規約」だけをタップして `https://yuzu.style/terms` が開くか（周囲の地の文をタップして誤発火しないか）— iOS Simulator（iPhone 17 Pro / iOS 26.5）で検証済み 2026-07-20。リンクタップで Safari が terms を開き、地の文タップでは遷移しないことを確認
- [x] 同様に「プライバシーポリシー」だけをタップして `https://yuzu.style/privacy` が開くか — 同上、Simulator で検証済み
- [ ] 両リンクとも haptics（tapLight）が鳴るか（Simulator では鳴らないため物理 iPhone が必要）
- [x] 注記の折り返し・中央揃えが小さい画面幅（SE等）でも崩れないか — コードレビューでクローズ（2026-07-20）。`numberOfLines`・固定高の指定が無く `textAlign: center` のみのため、SE 幅（375pt）では最悪2行の中央揃えに折り返すだけでクリップ・重なりは構造上発生しない。iPhone 17 Pro 幅では1行表示を Simulator で確認済み

録音カウントダウンの表示バグを修正（`components/RecordModal.tsx`）。`finishRecording` が `recordingStartedAt` を null にしてから phase が carving に進むまでの await 区間で、elapsed が 0 にリセットされ「1:00」（満タン表示）が一瞬出ていた。カウントダウンと進捗リングの描画条件を `isRecording && recordingStartedAt !== null` に変更:

- [ ] 録音を最大時間（1:00）まで放置して自動停止したとき、0:00 到達後に「1:00」が一瞬表示されずそのまま CARVING に遷移するか
- [ ] 手動で指を離したときもカウントダウン・リングが即座に消え、満タン表示のフラッシュが無いか

TestFlight Build 22 の実機フィードバック4件への対応（SIGNALウィジェット色反転・ヒートマップ16週固定・録音haptics・EMOTIONグラデ復帰+ゆず黄化）。ウィジェットはExpo Goでは動かず、hapticsはシミュレータで鳴らないため実機（次回TestFlightビルド）での確認が必要:

- [ ] SIGNAL ウィジェット: 録音直後にゆず黄が灯り、時間経過（〜48h）で徐々にインク色へ消えていくか。**録音後もずっと黒いままなら App Group entitlement が実機バイナリに効いていない**ことが確定するので要調査
- [ ] INSIGHT > SIGNAL ヒートマップ: 常に16週分の列で表示され、セルが約17px（グリッド高さ ~140px、従来の半分以下）に収まっているか。初ログ以前の期間が divider 色で自然に埋まっているか
- [ ] 録音中 20秒・40秒地点で軽いタックが鳴るか。**iOS は録音セッション中に触覚を抑制する挙動が既知のため（録音後に触覚が死ぬ件を audio session 解除で直した経緯あり）、実機で鳴らない可能性あり** — 鳴らなければ要再設計
- [ ] 記録保存の完了時（CARVED）に「Success通知→軽→中」の連鎖ハプティクス（`haptics.celebrate()`）がめでたい感じに鳴るか（通常保存・ログイン後のpending保存の両経路）
- [ ] EMOTION チャート・REPORTS ミニ sparkline がグラデーション（ゼロラインに向かって薄くなる）+ ポジティブ側ゆず黄（#F5D84A）で表示され、薄い側が背景に溶けて見えないか。LOG 投稿カード左端バー・REPORTS エッジバーもゆず黄系に変わる（意図的、DESIGN.md 冒頭の差分リスト参照）
...
- [ ] App Store Connect 側でアプリレコード作成・プライバシーポリシー URL の登録（メールアドレス・音声データを扱うため必須。公開URLは `https://yuzu.style/privacy` / `https://yuzu.style/terms` で確定済み（2026-07-20）。残るは ASC 側への入力のみ。[yuzu-app#105](https://github.com/studiocon/yuzu-app/issues/105)）
...
- [x] yuzu-app リポジトリの Supabase マイグレーション `20260702075418_report_jobs.sql` と `20260702130000_anon_stt_rate_limit.sql` は本番適用済み（`supabase migration list --linked` で Local/Remote 一致を確認、`anon_stt_usage`/`increment_anon_stt_usage`/`report_jobs` の実体・RLS権限も直接クエリで検証済み。2026-07-21）。匿名STTのIPベースレート制限は有効
...
- [x] プライバシーポリシー・利用規約の公開URL確定（`https://yuzu.style/terms` / `https://yuzu.style/privacy`、[yuzu-app#105](https://github.com/studiocon/yuzu-app/issues/105)）→ 設定画面の LEGAL セクション復活・サインイン画面への同意注記追加も実施済み（2026-07-20。実機確認は「実機検証待ち」参照）。ASC 側へのプライバシーポリシー URL 登録は上記 TestFlight チェックリスト参照
...
- 設定画面の LEGAL セクションはページ公開に伴い復活済み（2026-07-20）。旧 YOU（プラン）行は ACCOUNT の「種類」行、旧 ALERT は NOTIFICATIONS セクションとして実装済みで、審査対策の非表示は全て解消（[components/SettingsScreen.tsx](components/SettingsScreen.tsx)）
